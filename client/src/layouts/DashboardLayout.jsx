...
const DashboardLayout = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center">
        ...
        <p className="mt-4 text-slate-400 font-medium animate-pulse">Loading secure workspace...</p>
      </div>
    );
  }
    ...
    <div className="h-full flex flex-col justify-between py-6 px-4">
      <div>
        ...
        <div className="flex items-center gap-3 px-3 mb-8">
          ...
          <div>
            <h1 className="text-lg font-bold bg-gradient-to-r from-indigo-200 to-white bg-clip-text text-transparent">TalentSphere</h1>
            <p className="text-xs text-indigo-400 font-semibold tracking-wider uppercase">AI Platform</p>
          </div>
        </div>

        {/* Navigation links */}
        <div className="space-y-1.5">
          <p className="px-3 text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Practice Space</p>
          {navLinks.map((link) => {
            ...
            return (
              <Link
                ...
                className={`relative flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-200 group ${
                  isActive
                    ? 'text-white font-medium bg-indigo-600/25 border-l-4 border-indigo-500'
        ...
        {user.role === 'admin' && (
          <div className="mt-8 space-y-1.5">
            <p className="px-3 text-xs font-semibold text-rose-500 uppercase tracking-wider mb-2">Control Panel</p>
            {adminLinks.map((link) => {
              const isActive = location.pathname === link.path;
              return (
                <Link
                  ...
                  className={`relative flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-200 ${
                    isActive
                      ? 'text-white font-medium bg-rose-600/20 border-l-4 border-rose-500'
                      : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900/50'
                  }`}
                >
                  {link.icon}
                  ...
                  {isActive && (
                    <motion.div
                      ...
                      className="absolute right-3 w-1.5 h-1.5 rounded-full bg-rose-400"
                      transition={{ type: 'spring', stiffness: 300, damping: 30 }}
                    />
                  )}
                </Link>
      ...
      <div className="pt-6 border-t border-slate-900">
        <div className="flex items-center gap-3 px-3 py-2 bg-slate-900/40 rounded-2xl border border-slate-800/40 mb-4">
          <img
          ...
          <div className="overflow-hidden">
            ...
            <span className="text-[10px] bg-slate-800 text-indigo-400 border border-indigo-500/20 font-bold px-1.5 py-0.5 rounded-md uppercase">
              {user.role}
            </span>
          </div>
        </div>
        <button
          ...
          className="w-full flex items-center gap-3.5 px-4 py-3 text-slate-400 hover:text-rose-400 hover:bg-rose-950/20 rounded-xl transition-all duration-200"
        >
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex">
      {/* Sidebar - Desktop */}
      <aside className="hidden lg:block w-64 h-screen sticky top-0 bg-slate-950/80 border-r border-slate-900 backdrop-blur-md z-20 flex-shrink-0">
        <SidebarContent />
      </aside>

      {/* Mobile Top Header */}
      <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-slate-950/95 border-b border-slate-900 backdrop-blur-md z-30 px-6 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-slate-200">TalentSphere AI</span>
        </div>
        <button
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside
              ...
              className="lg:hidden fixed top-0 bottom-0 left-0 w-64 bg-slate-950 z-50 shadow-2xl shadow-indigo-500/5"
            >
              <SidebarContent />
            </motion.aside>
          </>
      ...
      <main className="flex-grow min-h-screen pt-16 lg:pt-0 overflow-y-auto bg-slate-950 relative">
