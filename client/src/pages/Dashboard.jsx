  ...
  if (loading) {
    return (
      <div className="space-y-8 animate-pulse">
        {/* Shimmer header */}
        <div className="h-10 w-1/3 bg-slate-900 rounded-xl shimmer"></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 h-96 bg-slate-900 rounded-3xl shimmer border border-slate-800/40"></div>
          <div className="h-96 bg-slate-900 rounded-3xl shimmer border border-slate-800/40"></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <Link
          to="/categories"
          className="flex items-center gap-2 px-5 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-sm font-semibold shadow-lg shadow-indigo-600/15 transition-all group shrink-0"
        >
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="glass-card rounded-3xl p-6 flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Questions Solved</span>
            <span className="text-3xl font-black text-slate-100 mt-2 block">{metrics.solvedCount}</span>
          </div>
        ...
        <div className="glass-card rounded-3xl p-6 flex items-center justify-between">
          <div>
            ...
            <span className="text-3xl font-black text-slate-100 mt-2 block">
        ...
        <div className="glass-card rounded-3xl p-6 flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Avg. Evaluation</span>
            <span className="text-3xl font-black text-slate-100 mt-2 block">{metrics.accuracy}%</span>
          </div>
        ...
        <div className="glass-card rounded-3xl p-6 flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Saved Questions</span>
            <span className="text-3xl font-black text-slate-100 mt-2 block">{metrics.bookmarkedCount}</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <motion.div variants={itemVariants} className="lg:col-span-2 glass-panel rounded-3xl p-6 flex flex-col justify-between min-h-[380px]">
          <div>
            <h3 className="text-lg font-bold text-slate-200 flex items-center gap-2">
              <FiAward className="text-indigo-400 w-5 h-5" /> Weekly Practice Volume
            </h3>
            <p className="text-xs text-slate-400 mt-1">Number of questions submitted daily over the last 7 days.</p>
          </div>
        ...
        <motion.div variants={itemVariants} className="glass-panel rounded-3xl p-6 flex flex-col justify-between">
          <div>
            <h3 className="text-lg font-bold text-slate-200">Difficulty Breakdown</h3>
            <p className="text-xs text-slate-400 mt-1">Distribution of questions solved by difficulty level.</p>
          </div>
            ...
            {metrics.solvedCount > 0 ? (
              <>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  <span className="text-2xl font-black text-slate-200">{metrics.solvedCount}</span>
          ...
          {/* Legend */}
          <div className="grid grid-cols-3 gap-3 border-t border-slate-900 pt-4 text-center mt-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <motion.div variants={itemVariants} className="lg:col-span-2 glass-panel rounded-3xl p-6">
          <h3 className="text-lg font-bold text-slate-200 mb-6">Topic Progress</h3>
          <div className="space-y-4">
            ...
            {roleProgress.length > 5 && (
              <div className="pt-2 text-center">
                <Link to="/categories" className="text-xs text-indigo-400 hover:text-indigo-300 font-semibold flex items-center justify-center gap-1">
        ...
        <motion.div variants={itemVariants} className="glass-panel rounded-3xl p-6 flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">Your latest submission attempts and score outcomes.</p>
          </div>
          <div className="mt-6 flex-grow space-y-4">
            {recentActivity.length > 0 ? (
              recentActivity.map((act) => (
                <div key={act._id} className="flex justify-between items-start gap-3 p-3 bg-slate-900/40 border border-slate-800/30 rounded-2xl hover:border-slate-800/80 transition-colors">
                  <div className="overflow-hidden">
                    <Link
                      to={`/categories/${act.role.toLowerCase().replace(/\s+/g, '-')}/${act.questionId}`}
                      className="text-sm font-semibold hover:text-indigo-400 transition-colors block truncate text-slate-200"
                    >
                      {act.title}
                    </Link>
          ...
          <div className="pt-4 border-t border-slate-900 mt-4">
            <Link to="/history" className="text-xs text-indigo-400 hover:text-indigo-300 font-semibold flex items-center justify-center gap-1">
