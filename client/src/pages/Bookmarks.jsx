    ...
    return (
      <div className="space-y-6 animate-pulse">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-44 bg-slate-900 rounded-3xl shimmer border border-slate-800/40"></div>
          ))}
        </div>
      </div>
    );
      ...
      <AnimatePresence mode="popLayout">
        {bookmarks.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {bookmarks.map((q, idx) => (
              <motion.div
                ...
                className="group bg-slate-900/40 border border-slate-800/50 hover:border-slate-700/60 p-5 rounded-3xl flex flex-col justify-between space-y-4 hover:shadow-lg hover:shadow-cyan-500/2"
              >
                <div className="space-y-3">
                  {/* Meta items */}
                  <div className="flex flex-wrap items-center gap-2 text-[10px] font-bold">
                    <span className="bg-slate-950 text-slate-400 px-2 py-0.5 rounded-md border border-slate-800/40 uppercase">
                      {q.role}
                    </span>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {q.tags.slice(0, 3).map((tag) => (
                      <span key={tag} className="text-[10px] bg-slate-950 text-slate-500 px-2 py-0.5 rounded-full border border-slate-800/20">
                        #{tag}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex justify-between items-center pt-3 border-t border-slate-900">
                  <button
                    ...
                    className="p-2.5 text-slate-500 hover:text-rose-400 hover:bg-rose-950/20 rounded-xl transition-all"
          ...
          <div className="glass-panel rounded-3xl py-14 text-center text-slate-500">
            <FiBookmark className="w-8 h-8 mx-auto text-slate-700 mb-3" />
